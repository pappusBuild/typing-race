...
const menuItems: ProfileDropdownItem[] = ["View Profile", "Account", "Setting", "Logout"]

function ProfileDropdown({ username, role, avatar, className, onItemSelect, ...props }: ProfileDropdownProps) {
        ...
        <div ref={containerRef} className={cn("relative inline-flex", className)} {...props}>
            ...
            >
                <span className="flex flex-col items-end gap-0.5 whitespace-nowrap pt-0.5">
                    <span className="text-sm font-bold text-base-white">{username}</span>
                    <span className="text-xs text-avatar-border">{role}</span>
                </span>
                <span className="flex size-12 items-center justify-center rounded-full border-2 border-avatar-border shadow-[0_0_5px_var(--avatar-border)]">
                    {avatar ?? <Avatar variant="profile" className="size-10 border-0" />}
                </span>
            </button>

            {isOpen && (
                <DropdownList role="menu" className="absolute right-0 top-full z-50 mt-2">
                    {menuItems.map((item, index) => (
                        <React.Fragment key={item}>
                            {index === 3 && <div className="mx-2 h-0.5 rounded-full bg-secondary-white" />}
                            <DropdownItem
                                role="menuitem"
